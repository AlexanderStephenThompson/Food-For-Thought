...
Usage:
    .venv/bin/python -m silver.build                    # full rebuild
    .venv/bin/python -m silver.build --check-idempotent # rebuild in memory,
                                                        # verify disk matches
...
from silver.pipeline.build_cuisines import build_cuisines_payload, load_cuisine_families
from silver.pipeline.build_vocabulary import (
    build_vocabulary_from_index,
    load_pipeline_lexicons,
    write_review_queue_to_path,
)
from silver.pipeline.compile_alias_table import (
    CompileStatistics,
    compile_ingredients_payload,
    load_merge_decisions,
    validate_compiled_payload,
)
from silver.pipeline.artifact_io import (
    ARTIFACT_JSON_INDENT,
    compute_build_fingerprint,
    write_artifact_json,
)
from silver.pipeline.load_bronze_recipes import (
    BRONZE_TRAIN_PATH,
    build_train_index,
    load_test_recipes,
    load_train_recipes,
)
from silver.pipeline.resolve_ingredient import IngredientResolver
from silver.pipeline.transform_bronze_to_silver import stage_recipes, write_silver_recipes
from silver.pipeline.validate_silver import validate_silver_artifacts
...
def write_silver_artifacts(artifacts: SilverArtifacts) -> None:
    """Persist every artifact atomically to silver/datasets/ and silver/reports/."""
