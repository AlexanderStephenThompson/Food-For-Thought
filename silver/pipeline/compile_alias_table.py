...
Takes the VocabularyBuild produced by silver.pipeline.build_vocabulary,
applies the human/LLM verdicts from the merge-decision JSONL, and emits the
pinned silver/datasets/ingredients.json artifact: one entry per alias-scope
ingredient with
...
DEFAULT_DECISIONS_PATH = locations.LEXICONS_DIRECTORY / "merge_decisions.jsonl"
DEFAULT_OUTPUT_PATH = locations.SILVER_DATASETS_DIRECTORY / "ingredients.json"
...
def compile_ingredients_payload(
    ...
    silver/datasets/ingredients.json structure sorted by id and alias.
...
def main() -> None:
    """CLI: compile, validate, and write silver/datasets/ingredients.json."""
